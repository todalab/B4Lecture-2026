...
sample2d_1_design_mat = np.append(sample2d_1_ndarray[:, 0:1], np.ones((sample2d_1_num_data, 1)), axis=1) # 計画行列 X　sample2d_1_ndarray[:, 0:1]のように0:1としないとベクトルになってしまう
sample2d_1_design_mat_T = sample2d_1_design_mat.T # 計画行列の転置 X^T
sample2d_1_meseared_vec = sample2d_1_ndarray[:, 1] # 実測値のベクトル y

sample2d_1_weight_vec = np.linalg.inv(sample2d_1_design_mat_T @ sample2d_1_design_mat) @ sample2d_1_design_mat_T @ sample2d_1_meseared_vec # 重みのベクトル w = (X^T X)^{-1} X^T y
...
sample2d_2_design_mat = np.append(sample2d_2_ndarray[:, 0:1], sample2d_2_ndarray[:, 0:1] ** 2, axis=1) # x^2の列を追加
for i in range(3, degree + 1):
    sample2d_2_design_mat = np.append(sample2d_2_design_mat, sample2d_2_design_mat[:, 0:1] ** i, axis=1) # x^iの列を追加
sample2d_2_design_mat = np.append(sample2d_2_design_mat, np.ones((sample2d_2_num_data, 1)), axis=1) # 定数項の列を追加
sample2d_2_design_mat_T = sample2d_2_design_mat.T # 計画行列の転置 X^T
sample2d_2_meseared_vec = sample2d_2_ndarray[:, 1] # 実測値のベクトル y

sample2d_2_weight_vec = np.linalg.inv(sample2d_2_design_mat_T @ sample2d_2_design_mat) @ sample2d_2_design_mat_T @ sample2d_2_meseared_vec # 重みのベクトル w = (X^T X)^{-1} X^T y
...
y = sample2d_2_weight_vec[0] * x + sample2d_2_weight_vec[1] * x ** 2 + sample2d_2_weight_vec[2] * x ** 3 + sample2d_2_weight_vec[3] # 3次の重回帰の式
...
sample3d_design_mat = np.append(sample3d_ndarray[:, 0:2], sample3d_ndarray[:, 0:2] ** 2, axis=1) # x^2, y^2の列を追加
sample3d_design_mat = np.append(sample3d_design_mat, sample3d_ndarray[:, 0:1] * sample3d_ndarray[:, 1:2], axis=1) # x*yの列を追加
sample3d_design_mat = np.append(sample3d_design_mat, np.ones((sample3d_num_data, 1)), axis=1) # 定数項の列を追加
sample3d_design_mat_T = sample3d_design_mat.T # 計画行列の転置 X^T
sample3d_meseared_vec = sample3d_ndarray[:, 2] # 実測値のベクトル z
sample3d_weight_vec = np.linalg.inv(sample3d_design_mat_T @ sample3d_design_mat) @ sample3d_design_mat_T @ sample3d_meseared_vec # 重みのベクトル w = (X^T X)^{-1} X^T y
...
X, Y = np.meshgrid(x, y)
Z = (sample3d_weight_vec[0] * X + sample3d_weight_vec[1] * Y + sample3d_weight_vec[2] * X ** 2 + sample3d_weight_vec[3] * Y ** 2 + sample3d_weight_vec[4] * X * Y + sample3d_weight_vec[5]) # 2次の重回帰の式
