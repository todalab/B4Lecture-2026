...
def main(cfg: DictConfig) -> None:
    ...
    for model_id in cfg.data.target_models:
        ...
        m.eval()
        models[model_id]     = m
        thresholds[model_id] = torch.load(thr_path)["threshold"] if thr_path.exists() else None
    ...
    with torch.no_grad():
        for wav_path in wav_files:
            ...
            segs  = process_file(
                str(wav_path),
                sr=cfg.data.sr, n_fft=cfg.data.n_fft,
                hop_length=cfg.data.hop_length, n_mels=cfg.data.n_mels,
