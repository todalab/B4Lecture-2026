...
> - PyTorchの基本機能のみを使用し、高レベルの VAE ライブラリは使用禁止（`pythae` 等の専用ライブラリや `torch.distributions.kl_divergence()` のような KL 解析計算 API を含む）

本課題では **Variational Autoencoder (VAE)** を実装し、**MNIST 手書き数字データセット**で学習する。  
...
深層学習における生成モデルは、データの確率的な構造を学習し、新たなサンプルを生成することを目的とする。しかし、観測データから潜在変数の真の事後分布を直接求めることは、一般に計算量的に困難である。
...
`VAEs/VAE.py` 内の `#TODO` を埋めて、以下の関数を完成させてください。  
...
| ファイル | 内容 |
|----------|------|
| `VAEs/VAE_skeleton.py` | 実装箇所（`#TODO`）が明示された VAE スケルトン |
...
Loss（= −ELBO）は学習とともに単調に減少し、収束に向かう。
...
- **音声への応用**: VAE は音声分野でも活用されている。例えば VAE-SiFiGAN（Shibuya et al., INTERSPEECH 2023）は VAE を用いた高品質音声合成ボコーダである。本課題で学んだ潜在変数モデルの考え方がどのように応用されているか調べてみよう。
...
- CPU でも実行可能（MNIST は軽量なため、デフォルト設定で数分以内に完了）
- GPU がある場合は数十秒で完了（`z_dim=2, h_dim=400` 程度）
- GPU メモリ使用量は `torch.cuda.max_memory_allocated() / 1024**2` [MB] で確認できる
