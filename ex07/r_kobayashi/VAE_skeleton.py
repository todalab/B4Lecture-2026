...
class VAE(nn.Module):
    ...
    def reparametrization_trick(
        ...
        # そのままサンプリングしようとすると微分ができないので、torch.randn_like() を使って mean と同じ形状の標準正規分布からサンプリングする。
        eps = torch.randn_like(mean)  # 平均と同じ形状の標準正規分布からサンプリング
        z = mean + eps * torch.exp(
            0.5 * log_var
        )  # Reparametrization trick を適用して z を計算
        return z
