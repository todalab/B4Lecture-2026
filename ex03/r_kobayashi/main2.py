...
def main(arg, k):
    ...
    Returns:
        log_list (ndarray of shape (T,)): 対数尤度の一覧(Tは反復回数)
        N (int): データ数
        p (int) : パラメータ数
    """
    arg = str(arg)
    ...
    #     alpha=0.5,
    # )
    # plt.savefig(fig_name + "_k" + str(k))
    # plt.show()

    # with open(csv_name + "_k" + str(k), "w") as file:
    #     writer = csv.writer(file, lineterminator="\n")
    #     writer.writerows(gamma.T)

    # with open(dat_name + "_k" + str(k), "w") as file:
    #     writer = csv.writer(file, lineterminator="\n\n")
    ...
    # plt.savefig(fig_gap_name + "_k" + str(k))
    # plt.show()
